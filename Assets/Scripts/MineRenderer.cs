...
public class MineRenderer : MonoBehaviour
{
    ...
    public void CreateTiles(int chunkRow)
    {
        ...
        for (int i = -50; i != 50; i += 25) {
            ...
            int oreVeinCount = Random.Range(2, 5);
    ...
    private void GenerateOreVeins(Tilemap mineTilemap, int veinCount, int chunkX, int chunkRow)
    {
        for (int v = 0; v < veinCount; v++)
        {
            ...
            int radius = Random.Range(3, 5); // Radius of 3-5 tiles for variation
            ...
            for (int x = -radius; x <= radius; x++)
            {
                for (int y = -radius; y <= radius; y++)
                {
                    ...
                    if (distanceFromCenter <= radius)
                    {
                        ...
                        {
                            Vector3Int tilePosition = new(chunkX + tileX, (chunkRow - 1) * -25 - 5 - tileY, 0);
    ...
    private TileBase SelectOreBasedOnDepth(int chunkRow)
    {
        ...
        float depthFactor = Mathf.Clamp01(chunkRow / 10f);  // Lower 5f to make the rarity change faster, increase to change it slower
